...
expect(shape.render()).toEqual('<polygon points="100, 15 200, 200 0, 200" fill=""/>')
    });
});
...
shape.setColor("");
// Assert that the render method returns the correct SVG string
expect(shape.render()).toEqual('<circle cx="150" cy="100" r="100" fill=""/>')
    });
});
...
shape.setColor("");
// Assert that the render method returns the correct SVG string
expect(shape.render()).toEqual('<rect width="200" height="200" fill=""/>')
    });
});